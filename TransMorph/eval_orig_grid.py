...
def main():
    ...
    hd_metric = HausdorffDistanceMetric(
        include_background=False, percentile=100, reduction="mean"
    )
    ...
    with torch.no_grad():
        for idx, row in df.iterrows():
            ...
            true_fixed_seg_path = get_true_original_path(
                row.get("fixed_seg_path", None)
            )

            # Original moving image
            true_moving_img_path = get_true_original_path(row["moving_img_path"])
            true_moving_seg_path = get_true_original_path(
                row.get("moving_seg_path", None)
            )

            # Resampled predictions
            moved_img_path = get_mirror_path(
                row["moving_img_path"], row["fixed_img_path"], suffix="_moved_orig_grid"
            )
            moved_seg_path = get_mirror_path(
                row["moving_seg_path"], row["fixed_seg_path"], suffix="_moved_orig_grid"
            )

            if not os.path.exists(moved_img_path) or not os.path.exists(
                true_fixed_img_path
            ):
            ...
            fixed_img_t = (
                torch.tensor(true_fixed_img, dtype=torch.float32, device=device)
                .unsqueeze(0)
                .unsqueeze(0)
            )
            moved_img_t = (
                torch.tensor(moved_img, dtype=torch.float32, device=device)
                .unsqueeze(0)
                .unsqueeze(0)
            )
            ...
            if (
                true_fixed_seg_path
                and os.path.exists(true_fixed_seg_path)
                and os.path.exists(moved_seg_path)
            ):
                ...
                fixed_seg_t = (
                    torch.tensor(true_fixed_seg, dtype=torch.int64, device=device)
                    .unsqueeze(0)
                    .unsqueeze(0)
                )
                moved_seg_t = (
                    torch.tensor(moved_seg, dtype=torch.int64, device=device)
                    .unsqueeze(0)
                    .unsqueeze(0)
                )
                ...
                num_classes = (
                    max(int(fixed_seg_t.max().item()), int(moved_seg_t.max().item()))
                    + 1
                )
                ...
                if num_classes > 1:
                    ...
                    hd_metric(
                        y_pred=moved_seg_onehot, y=fixed_seg_onehot, spacing=spacing
                    )
                    ...
                    print(
                        f"Case {idx} has no foreground segmentation classes. Skipping Dice/HD."
                    )

            else:
                print(
                    f"Fixed seg path is {os.path.exists(true_fixed_seg_path)}: {true_fixed_seg_path}, Moved seg path is {os.path.exists(moved_seg_path)}: {moved_seg_path}"
                )
            ...
            print(
                f"Case {idx:03d} | Dice: {dice_val:.4f} | HD: {hd_val:.4f}mm | SSIM: {ssim_val:.4f}"
            )
