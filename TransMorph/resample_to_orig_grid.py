...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if not os.path.exists(true_fixed_img_path):
            print(f"Warning: True fixed image not found at {true_fixed_img_path}. Skipping.")
            continue

        true_fixed_nii = nib.load(true_fixed_img_path)

        # Get existing 160^3 outputs
        moved_img_path = get_mirror_path(moving_img_path_csv, fixed_img_path_csv, suffix='_moved')
        moved_seg_path = get_mirror_path(moving_seg_path_csv, fixed_img_path_csv, suffix='_moved')

        # New paths for ITK-SNAP compatible files
        itk_img_path = get_mirror_path(moving_img_path_csv, fixed_img_path_csv, suffix='_moved_orig_grid')
        itk_seg_path = get_mirror_path(moving_seg_path_csv, fixed_img_path_csv, suffix='_moved_orig_grid')
        ...
        print(f"Case {idx:03d} -> Accurately resampled to physical shape: {true_fixed_nii.shape}. Fixed: {true_fixed_img_path}. Moved: {moved_img_path}")

    print("-" * 40)
    print("Resampling Complete! Your bodies should no longer look squished in ITK-SNAP.")
